...
**Architecture:** Keep meeting state in `pages/app/sessions/[id]/room.vue`, because the current Agora room state is already localized there. Use Agora RTC stream messages for in-session-only chat, emoji reactions, and raised-hand state. Guard the global Nautix launcher in `app.vue` for room routes.
...
- [x] **Step 5: Hide external support launcher on room routes**

Guard `NautixWebChat` in `app.vue` so the global launcher remains available elsewhere and does not overlap the meeting controls.

### Task 3: Verify

**Files:**
- Test: `tests/agora-session-meetings.test.mjs`
- Test: `tests/nautix-webchat.test.mjs`
...
- [x] **Step 2: Run Nautix guard regression test**

Run: `node tests/nautix-webchat.test.mjs`

Expected: PASS.

- [x] **Step 3: Run Nuxt build**
