<template>
  <DialogContent class="sm:max-w-[600px]">
    <DialogHeader>
      ...
      <DialogDescription>
        Update settings for this sync schedule.
      ...
      <Tabs defaultValue="basic" class="w-full">
        ...
        <TabsContent value="basic" class="space-y-4">
          <div class="space-y-4">
            ...
            <div v-if="scheduleData.type === 'realtime'" class="space-y-2">
              <div class="flex items-center space-x-2">
                <Checkbox 
                  id="optimizationReviewed" 
                  v-model:checked="scheduleData.optimizationReviewed"
                />
                <Label for="optimizationReviewed">Mark as performance reviewed</Label>
              </div>
              <p class="text-xs text-muted-foreground">
                This will suppress performance optimization suggestions for this schedule
              </p>
            </div>
          </div>
        </TabsContent>
        ...
        <TabsContent value="scope" class="space-y-4">
          ...
          <div v-if="scheduleData.scope === 'category'" class="space-y-2">
            ...
            <Multiselect
              ...
              :class="{ 'border-destructive': validationErrors.categoryIds }"
            />
          ...
          <div v-if="scheduleData.scope === 'product'" class="space-y-2">
            ...
            <Multiselect
              ...
              :class="{ 'border-destructive': validationErrors.productIds }"
            />
          ...
          <div v-if="scheduleData.scope === 'item'" class="space-y-2">
            ...
            <Multiselect
              ...
              :class="{ 'border-destructive': validationErrors.itemIds }"
            />
            ...
            <div class="space-y-2">
              <div class="flex items-center space-x-2">
                <Checkbox 
                  ...
                  v-model:checked="scheduleData.notifications.enabled"
                />
                <Label for="enableNotifications">Enable notifications</Label>
              </div>
              
              <div v-if="scheduleData.notifications.enabled" class="space-y-2 pl-6">
...
// Replace the MultiSelect import with vue-multiselect
...
const emit = defineEmits(['schedule-updated', 'close'])
...
// Schedule data with defaults for any missing fields
...
onMounted(() => {
  // Make a deep copy to avoid modifying props directly
  const schedule = JSON.parse(JSON.stringify(props.schedule))
  ...
  if (!schedule.errorHandling) {
    schedule.errorHandling = {
      stopOnError: false,
      logErrors: true,
      alertOnError: true
    }
  ...
  // Make sure collections are arrays
  schedule.tags = schedule.tags || []
  schedule.categoryIds = schedule.categoryIds || []
  schedule.productIds = schedule.productIds || []
  schedule.itemIds = schedule.itemIds || []
  schedule.notifications.channels = schedule.notifications.channels || []
...
function isChannelSelected(channel) {
  return scheduleData.value.notifications?.channels?.includes(channel) || false
}

function toggleChannel(channel, checked) {
  if (!scheduleData.value.notifications) {
    scheduleData.value.notifications = { enabled: true, channels: [] }
  }
  
  if (!scheduleData.value.notifications.channels) {
    scheduleData.value.notifications.channels = []
  }
  
  if (checked) {
    if (!scheduleData.value.notifications.channels.includes(channel)) {
      scheduleData.value.notifications.channels.push(channel)
    }
  } else {
    scheduleData.value.notifications.channels = 
      scheduleData.value.notifications.channels.filter(c => c !== channel)
  }
}
...
async function submitForm() {
  ...
  try {
    ...
    emit('schedule-updated', finalData)
  } catch (error) {
