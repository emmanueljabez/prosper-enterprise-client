<template>
  <DialogContent class="sm:max-w-[700px] max-h-[90vh] flex flex-col">
    <DialogHeader class="px-6 pt-6">
      ...
      <DialogDescription>
        Configure when and how inventory changes are synchronized with the catalog
      </DialogDescription>
    </DialogHeader>
    
    <div class="flex-1 overflow-y-auto px-6">
      <form @submit.prevent="submitForm" id="create-rule-form" class="py-4">
    ...
    <DialogFooter class="px-6 py-4 border-t">
      ...
      <Button type="submit" form="create-rule-form" :disabled="isSubmitting">
...
const emit = defineEmits(['rule-created', 'close'])
...
function handleActionChange(action) {
  ...
  formData.value.actionConfig = {
    notifyStaff: formData.value.actionConfig.notifyStaff,
...
async function submitForm() {
  ...
  try {
    // Emit the rule created event with the form data
    emit('rule-created', { ...formData.value })
  } catch (error) {
