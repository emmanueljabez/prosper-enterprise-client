...
import { Users, Search, RefreshCw, UserPlus, Trash2, Layers3 } from 'lucide-vue-next'

definePageMeta({
  title: 'Employees',
  description: 'Enroll employees into company programs and manage employee rosters',
  requiresAuth: true,
...
const enrollSelectedEmployees = async () => {
  if (!selectedProgramId.value) {
    toast.error('Select a company program first')
    return
  }
  ...
  try {
    await companyProgramsStore.enrollProgramParticipants(selectedProgramId.value, selectedProfileIds.value)
...
const removeParticipant = async (participantId: string) => {
  if (!confirm('Remove this employee from the selected program?')) {
    return
  }
...
watch(employeeSearch, () => {
  ...
  employeeSearchDebounce.value = setTimeout(() => {
    loadEmployees()
  }, 350)
})
...
watch(participantSearch, () => {
  ...
  participantSearchDebounce.value = setTimeout(() => {
    loadParticipants()
  }, 350)
})

watch(participantStatus, () => {
  loadParticipants()
})
</script>
  ...
  <div class="container mx-auto space-y-6 px-4 py-6">
    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
      <div>
        ...
        <p class="text-sm text-muted-foreground">
          Enroll company employees into a mentorship program and manage the active roster.
        </p>
      </div>

      <div class="flex gap-2">
        <Button variant="outline" @click="loadParticipants" :disabled="participantsLoading || !selectedProgramId">
          <RefreshCw class="mr-2 h-4 w-4" :class="{ 'animate-spin': participantsLoading }" />
          Refresh roster
        </Button>
        <Button @click="enrollSelectedEmployees" :disabled="participantsSaving || !selectedCount || !selectedProgramId">
    ...
    <div class="grid gap-4 md:grid-cols-3">
      <Card>
        <CardHeader class="pb-2">
          ...
          <CardTitle class="text-3xl">{{ programs.length }}</CardTitle>
        </CardHeader>
      </Card>
      <Card>
        <CardHeader class="pb-2">
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>Select the company program you want to manage.</CardDescription>
      </CardHeader>
      <CardContent class="space-y-4">
        ...
        <div v-else class="grid gap-3 md:grid-cols-[minmax(0,1fr),220px]">
          ...
          <Button variant="outline" @click="loadPrograms" :disabled="isLoading">
            <Layers3 class="mr-2 h-4 w-4" :class="{ 'animate-spin': isLoading }" />
            Refresh programs
          </Button>
        </div>
    ...
    <div class="grid gap-6 xl:grid-cols-[1.1fr,1fr]">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select employees from your company and enroll them into the chosen program.</CardDescription>
        </CardHeader>
        <CardContent class="space-y-4">
          <div class="relative">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Track who is already attached to the selected company program.</CardDescription>
        </CardHeader>
        <CardContent class="space-y-4">
