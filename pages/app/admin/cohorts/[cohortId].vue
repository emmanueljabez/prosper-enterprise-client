...
const cohortProgramId = computed(() => selectedCohort.value?.companyProgramId || '')
...
const loadProgramRoster = async () => {
  if (!selectedCohort.value?.companyProgramId) return
  try {
    await companyProgramsStore.loadProgramParticipants({
      companyProgramId: selectedCohort.value.companyProgramId,
      page: 0,
      size: 500,
      status: 'ALL',
    })
...
<template>
  <div class="container mx-auto space-y-6 px-4 py-6">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
      <div class="space-y-1">
        <Button variant="ghost" class="w-fit px-0 text-muted-foreground" @click="router.push('/app/admin/cohorts')">
    ...
    <CompanyProgramCohortEditorDialog
      ...
      :program-id="cohortProgramId"
      :company-id="selectedCohort.companyId || ''"
